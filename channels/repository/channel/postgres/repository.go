...
func (r *repository) Save(ctx context.Context, channel channel.Channel) error {
	...
	_, err = r.database.NamedExecContext(
		ctx,
		"INSERT INTO channels VALUES (:id, :name, :tenancy, :created_at, :deleted_at) ON CONFLICT (id) DO UPDATE SET name=:name, tenancy=:tenancy, created_at=:created_at, deleted_at=:deleted_at",
		model,
	)
	if err != nil {
